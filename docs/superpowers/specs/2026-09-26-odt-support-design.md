...
| 5 | Markdown via automatic styles, `EnableMarkdown` and `:raw`. `ValidateTemplate` for ODT (`OdtTemplateValidator`: the same rules, messages and loop scoping as `TemplateValidator`/`ScopedVariableValidator`, over the ODT traversal). `DocumentProperties` → `meta.xml`, with `meta.xml` created if it is missing. Mapping: Author → `meta:initial-creator`, LastModifiedBy → `dc:creator`, Title/Subject/Description → `dc:*`, Keywords → one `meta:keyword`, Category → the user-defined property `Category`, since ODF has no category field. |
...
- The preview thumbnail (`Thumbnails/thumbnail.png`) is removed, because it shows the template. `meta:document-statistic` is kept as is (LibreOffice recounts on save).
...
- The scoped-variable logic of `OdtTemplateValidator` duplicates `ScopedVariableValidator` (item property aggregation, loop scopes), because the DOCX code is kept untouched. A later refactoring can extract the shared, format-agnostic part.
